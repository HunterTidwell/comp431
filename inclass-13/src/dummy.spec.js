...
describe('Validate login', () => {
	...
	it('should update the headline', (done) => {
		...
		mock(`${url}/headline`, {
			...
			headers: {'Content-Type': 'application/json'},
			json: {
				headlines: [{username: 'my name is', headline:'slim shady'}]
			}
		})
		
		login().then(_ => {
			expect(div.innerHTML).to.eql('you are logged in as hi "ok"')
			updateHeadline("new headline! wow!").then(_ => {
				expect(div.innerHTML).to.eql('you are logged in as my name is "slim shady"')
			})
		})
		.catch(done)
		.then(done)
